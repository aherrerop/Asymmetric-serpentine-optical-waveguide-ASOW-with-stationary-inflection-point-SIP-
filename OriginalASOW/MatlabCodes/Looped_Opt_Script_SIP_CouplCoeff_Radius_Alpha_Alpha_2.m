...
%     CouplCoeff_k2_seed = CouplCoeff_min + rand()*(CouplCoeff_max-CouplCoeff_min);
    ...
    Radius_max = 11e-6;
    %
    Radius_seed = Radius_min + rand()*(Radius_max-Radius_min);
    
    % Initial guess (randomly assigned between X0_min & X0_max).
    X0 = [CouplCoeff_k1_seed, Alpha_seed, Alpha_2_seed, Radius_seed];
...
% Frequency Sweep
Freqmax = Freq_Center + 1e-1;
Freqmin = Freq_Center - 1e-1;
Freqsteps = 5000;
...
for jj = 1:NumberOfIterations
    [Sigma_min, index] = min(Sigma_EPD(jj));
    Sigma_min;
    Eigenvalues_min = Eigenvalues_EPD(:,index);
    X_min = X_EPD(:,index);
...
%     pC = X_min(5);
    ...
    for ii = 1:Freqsteps
        TransferMatrices_Vec(:,:,ii) =  get_UnitCell_3_PhaseDelay_1CouplCoeff_Parameters (Freq, CouplCoeff_k1, Radius, Alpha, Alpha_2);
    end
    [EigenVectors,EigenValues] = eigenshuffle(TransferMatrices_Vec);
    for ii = 1:Freqsteps
        Eigenvalues(:,ii) = -log(EigenValues(:,ii))./(1j);
